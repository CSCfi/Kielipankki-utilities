...
"""
Päivitykset
18.12.2015 lisätty unicode-lainausmerkkejä ja estetty virkejako
           pilkusta tapauksissa: "Sanoiko Matti niin?", hän kysyi.
...
#smileys = r'([\.\?!]?[:;x=]-*[DEPO\(\)\|codpf]+)|\b(=?[\^oOx;0]_+[\^oOx;-]=?\b)|[ \b]:3'
#char_map = r'(%s|[\.;:,\?!]+[»—\"\'\(\)\<\>\{\}]|(^|\s)-)' % smileys
#us_num = r'(\d+\.)+\d+(,\d\d)?|\d+,\d\d'
...
def truncate(string, maxlen):
    """ Typistä touretteripulit ja muut floodaukset n (maxlen) merkkiin"""
    l = len(string)
    truncated = str()
    i = 0
    for c in string:
...
def split_sents(string, maxlen, sent_id):
    ...
    #purkkakorjaus kts päivitys 18.12.2015.
    ...
    if sent_id is not None:
        t = [re.sub('<sentence_id', '<sentence id', token) for token in tokens]
        tokens = t

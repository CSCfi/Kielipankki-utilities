...
### 1. For each new resource (or a new version of an existing resource), create an "Epic" issue on Jira. 
   ...
   - NB: Both the Epic Name and Summary fields of the Jira Epic issue should contain identical text (= the name mentioned above).
...
### 3. In the Epic, create a "Story" for each of the applicable task list sections which were previously copied to the Epic description. 
  ...
  - In case the resource is a completely new one and it has not been decided what should be done with it, just create the first story ("[_shortname:_ Enter the new resource to the pipeline] (https://github.com/CSCfi/Kielipankki-utilities/blob/master/docs/corpus_publishing_tasklist.md#shortname-enter-the-new-resource-to-the-pipeline)").

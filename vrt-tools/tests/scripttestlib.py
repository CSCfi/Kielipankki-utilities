...
def _get_expected_value(expected, tmpdir, env):
    ...
    if isinstance(exp_val, dict):
        if len(exp_val) != 1:
            raise ValueError(
                'a dict as an expected value must have exactly one key')
        if 'python' in exp_val:
            return _exec_python_func(exp_val['python'], None)
        elif 'shell' in exp_val:
            return _exec_shell(exp_val['shell'], '', tmpdir=tmpdir, env=env)[0]
        elif 'file' in exp_val:
            return _get_file_content(exp_val['file'], tmpdir)
        else:
            raise ValueError(
                'a dict as an expected value with key other than "file",'
                f' "python" or "shell": {list(exp_val.keys())[0]}')
    return exp_val
